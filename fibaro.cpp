...
)
{
	...
	case Notification::Type_ValueChanged:
	{
		...
		if( NodeInfo* nodeInfo = GetNodeInfo( _notification ) )
		{
			...
			ValueID _val = _notification->GetValueID();
			switch( _val.GetType() )
			{
				case ValueID::ValueType_Bool:
				{
					bool o_value;
					Manager::Get()->GetValueAsBool( _val, &o_value );
					printf("TEST Bool %s\n", (o_value)?"true":"false");	// placeholder for real action
					break;	
				}
				...
				{
					string stringValue;
					Manager::Get()->GetValueAsString( _val, &stringValue );
					printf("TEST Decimal %s\n", stringValue.c_str());
					break;	
				}
...
int main( int argc, char* argv[] )
{
	...
	Options::Create( "./open-zwave-read-only/config/", "", "" );
...
//	Options::Get()->AddOptionInt( "DumpTrigger", LogLevel_Error );
	...
	if( !g_initFailed )
	{
		...
		for( list<NodeInfo*>::iterator it = g_nodes.begin(); it != g_nodes.end(); ++it )
		{
			...
			{
				ValueID v = *it2;
				printf(".....Value label = %s , unit = %s, node %o, node Product Type = %s, isReadOnly: %s, isWriteOnly: %s \n", Manager::Get()->GetValueLabel(v).c_str(), Manager::Get()-> GetValueUnits(v).c_str(), nodeInfo->m_nodeId, Manager::Get()->GetNodeType(nodeInfo->m_homeId,nodeInfo->m_nodeId).c_str(), (Manager::Get()->IsValueReadOnly(v))?"true":"false", (Manager::Get()->IsValueWriteOnly(v))?"true":"false");
				if (Manager::Get()->GetValueLabel(v) == "Switch"){
